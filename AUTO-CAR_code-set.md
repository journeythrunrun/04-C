...
- 나중에 코드 다시 필요하게 되면 떠올리기용으로 작성하는 코드에 대한 메모
  ...
  + 챗쥐피티한테 물어서 몇 번 해도 안나오면 챗 쥐피티가 그 부분 약하다는 거니까 걍 직접 처음부터 다시 검토하는 게 빨랐을 것임. 물론 몇 번 더 잘 물어서 찾아내는 경우도 있기에 뭐가 더 빠를지가 항상 같은 건 아닐 것임.
    > 배경 : 처음 써보는 것도 있어서 시간 절약해서 빨리 해결할 겸 어떤 걸 Chat gpt 유료버전에게 물어봤었는데, 생각보다 Chat gpt가 틀린 부분을 못찾기도 했음.( 지금 언급하는 경우는 활용적인 어려운 거라기보다 기초 설정 같은 거였음. 걍 모든 레지스터 비트당 설정 다 외우고 있진 않으니까 빨리 하려고도 물어봤었음.)  설정할 때 비트당 의미가 문서에(매뉴얼) 큰 차이로 담겨있어서 그런가. 실시간이 중요한 것에서 여러 개들 사이에 복합적인 타이밍 계산은 잘 못하는 건가. 물론 AI가 더 똑똑해지면 더 잘하게 될 수도 있지만 현재는 그러함. 틀린 게 자명한 게 있는데 관련해서 정답을 여러 번 물어도 틀리기도 했었음. 물론 잡아낼 수 있는 부분도 있겠고 유용한 도구임. 
