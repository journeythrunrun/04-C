...
### 1.2 7segment 분초시계, 시계/반시계방향 회전, FSM  (&관련 사소한 실행 코드 & 이전 코드 어느정도 누적 & 주석 일부 추가)
- 4자리 7segment, 분할 컴파일
  + main.c
    ```C
    // - 분&초 시계, FSM (버튼0으로 state 회전)
  ...
  + fnd.c
    ...
    void fnd_display(int state, int sec_count){
        ...
        // 두 방향의 회전 : % 18
        // 누적합을 데이터 포트값에다 더하며 하기엔 옆에서도 같이쓰는 data port니까 따로 변수로 저장해서 전달. 기본 분초시계에서도 FND_DATA_PORT = fnd_font[ 그렇게 했었음
